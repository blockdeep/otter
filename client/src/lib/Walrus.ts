...
export class WalrusClient {
  ...
  // Default endpoints
  private static readonly DEFAULT_ENDPOINTS: Record<
    WalrusNetwork,
    NetworkConfig
  > = {
  ...
  constructor(config: WalrusConfig) {
    ...
    this.aggregator =
      config.customEndpoints?.aggregator || defaultConfig.aggregator;
    this.publisher =
      config.customEndpoints?.publisher || defaultConfig.publisher;

    if (!this.publisher && this.network === WalrusNetwork.MAINNET) {
      console.warn(
        "No publisher endpoint specified for mainnet. You will not be able to upload blobs.",
      );
    }
  }
  ...
  public switchNetwork(
    network: WalrusNetwork,
    customEndpoints?: { aggregator?: string; publisher?: string },
  ) {
...
export function createWalrusClient(
  network: WalrusNetwork = WalrusNetwork.TESTNET,
): WalrusClient {
