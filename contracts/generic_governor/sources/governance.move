module generic_governor::governance {
    ...
    use generic_governor::govtoken::{GOVTOKEN};
    ...
    ctx: &mut TxContext,
        ...
        match (&proposal.kind) {
            ProposalKind::Increment => {
                simple_counter::increment(counter)
            },
            ProposalKind::SetValue { value } => {
                simple_counter::set_value(counter, *value)
            }
        };
    ...
    /// Get the proposal kind
    public fun get_proposal_kind(
        self: &GovernanceSystem,
        proposal_id: ID,
    ): &ProposalKind {
        assert!(table::contains(&self.proposals, proposal_id), EProposalNotFound);
